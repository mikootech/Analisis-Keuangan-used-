...
@st.cache_resource
def get_supabase():
    # Coba dari env dulu (dotenv), fallback ke st.secrets
    ...
    if not url or not key:
        raise ValueError("SUPABASE_URL dan SUPABASE_KEY harus diisi di .env atau secrets.toml!")
...
try:
    ADMIN_USERNAME = st.secrets.get("ADMIN_USERNAME", ADMIN_USERNAME)
except (KeyError, FileNotFoundError):
    pass

ADMIN_PASSWORD = os.getenv("ADMIN_PASSWORD", "").strip() or "admin102938182930"
try:
...
def ensure_admin_exists():
    ...
    try:
        r = sb.table("users").select("id,password").eq("username", ADMIN_USERNAME).execute()
        if not r.data:
